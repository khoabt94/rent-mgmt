...
export function AccountInfoPage() {
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      <h3 className="text-xl text-center">Xin chào <span className="font-bold capitalize">{user?.name}</span>!</h3>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-y-4 mt-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Input placeholder="Email của bạn" disabled {...field} className='text-base h-[50px]' />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <Button type="submit" className='mt-5' disabled={form.formState.isSubmitting}>Cập nhật</Button>
        </form>
      </Form>
